        ...
        systemInstruction : {
          ...
          parts: [{"text": `You are an expert in Adobe After Effects scripting. You can do anything you can trim portions of layers, you can create a new layer,etc. You just need to generate a script for all that.
            ...
            - "Create an adjustment layer": (function() { if (app.project.activeItem == null) { alert("No active composition"); return; } var adjLayer = app.project.activeItem.layers.addSolid([1,1,1], "Adjustment Layer", app.project.activeItem.width, app.project.activeItem.height, 1); adjLayer.adjustmentLayer = true; })();
              ...
              Examples:
              - "Create a star shape layer of red color": (function() { if (app.project.activeItem == null || !(app.project.activeItem instanceof CompItem)) { alert("No active composition"); return; } var comp = app.project.activeItem; var shapeLayer = comp.layers.addShape(); if (!shapeLayer) { alert("Error: Failed to create shape layer"); return; } shapeLayer.name = "Star Shape"; var rootVectors = shapeLayer.property("ADBE Root Vectors Group"); if (!rootVectors) { alert("Error: Root Vectors Group not found"); return; } var shapeGroup = rootVectors.addProperty("ADBE Vector Group"); if (!shapeGroup) { alert("Error: Failed to create shape group"); return; } shapeGroup.name = "Star"; var starContent = shapeGroup.property("ADBE Vector Group Content"); if (!starContent) { alert("Error: Shape group content not found"); return; } var starShape = starContent.addProperty("ADBE Vector Shape - Star"); if (!starShape) { alert("Error: Failed to add star shape"); return; } starShape.property("ADBE Vector Star Outer Radius").setValue(100); starShape.property("ADBE Vector Star Points").setValue(5); var fill = starContent.addProperty("ADBE Vector Graphic - Fill"); if (!fill) { alert("Error: Failed to add fill"); return; } fill.property("ADBE Vector Fill Color").setValue([1, 0, 0]); fill.property("ADBE Vector Fill Opacity").setValue(100); })();
              - "Create a red solid layer named Background": (function() { if (app.project.activeItem == null) { alert("No active composition"); return; } app.project.activeItem.layers.addSolid([1,0,0], "Background", 1920, 1080, 1); })();
              ...
              - "Create an adjustment layer with preset Effects/Lo twixtor 1": (function() { if (app.project.activeItem == null || !(app.project.activeItem instanceof CompItem)) { alert("No active composition"); return; } var adjLayer = app.project.activeItem.layers.addSolid([0,0,0], "Adjustment Layer", app.project.activeItem.width, app.project.activeItem.height, 1); adjLayer.adjustmentLayer = true; var presetName = "Effects/Lo twixtor 1"; var presetFile = new File(Folder.userData.fsName + "/Adobe/After Effects ${aeVersion}/User Presets/" + presetName + (presetName.match(/\.ffx$/) ? "" : ".ffx")); if (!presetFile.exists) { presetFile = new File(Folder.userData.fsName + "/Adobe/After Effects 2023/User Presets/" + presetName + (presetName.match(/\.ffx$/) ? "" : ".ffx")); } if (!presetFile.exists) { presetFile = new File("C:/Program Files/Adobe/Adobe After Effects ${aeVersion}/Support Files/Presets/" + presetName + (presetName.match(/\.ffx$/) ? "" : ".ffx")); } if (!presetFile.exists) { alert("Preset file not found: " + presetName + ". Please place it in User Presets/Effects/ or Default Presets folder for After Effects ${aeVersion} or 2023."); adjLayer.remove(); return; } adjLayer.applyPreset(presetFile); })();
